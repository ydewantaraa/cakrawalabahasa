...
<body class="bg-gray-100 font-sans text-[#151d52]">
    @php
        $priceModel = \App\Models\Price::with(['service', 'subService'])->find(request('price_id'));

        $qty = $priceModel && $priceModel->unit_type === 'per_item' ? (int) request('quantity', 1) : 1;

        if ($qty < 1) {
            $qty = 1;
        }

        $unitPrice = $priceModel ? (int) $priceModel->price : 0;
        $total = $unitPrice * $qty;
    @endphp
    <div class="max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden mt-10 p-6 text-center">
        ...
        <!-- Gambar Layanan -->
        <img src="{{ request('thumbnail') }}" alt="Foto Program"
            class="w-32 h-32 mx-auto rounded-lg mb-4 object-cover ring-1 ring-gray-200 shadow-sm">

        <!-- Info Program -->
        <h2 class="text-lg font-semibold mb-1">{{ request('service_name') }}</h2>
        @if (request('media'))
            <p class="text-sm text-gray-600 mb-1">
                <span class="font-semibold">Media:</span> {{ request('media') }}
            </p>
        @endif
        <p class="text-sm text-gray-600 mb-2"><span class="font-semibold">Sub Layanan:</span>
            {{ request('sub_service_name') }}
        </p>
        <p class="text-sm text-gray-600 mb-2">
            <span class="font-semibold">Jumlah:</span>
            {{ request('quantity', 1) }}
        </p>
        {{-- <p class="text-lg text-[#f78a28] font-bold mb-4">
        ...
        <div x-data="konfirmasiForm()" class="text-left space-y-4">
            ...
            <button type="button" @click="konfirmasi"
            ...
            document.getElementById('pay-button').addEventListener('click', function() {
                ...
                fetch("{{ url('/payment') }}", {
                        ...
                        body: JSON.stringify({
                            ...
                            layanan_id: "{{ Str::slug(request('service_name') . '-' . request('sub_service_name')) }}",
                            layanan: "{{ request('service_name') }}",
                            sub_layanan: "{{ request('sub_service_name') }}",
                            media: "{{ request('media') }}",
                            name: nama,
                            email: email,
                            phone: wa
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('konfirmasiForm', () => ({
                ...
                layanan: '{{ request('service_name') }}',
                media: '{{ request('media') }}',
                subLayanan: '{{ request('sub_service_name') }}',
                harga: '{{ request('price') }}',
                ...
                konfirmasi() {
                    ...
                    const url = `https://wa.me/${nomorAdmin}?text=${encoded}`;
