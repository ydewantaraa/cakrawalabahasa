        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 grid grid-cols-3 gap-6">
            @foreach($courses as $course)
                <div class="bg-white p-4 rounded shadow">
                    @if($course['img'])
                        <img src="{{ asset($course['img']) }}" alt="{{ $course['title'] }}" class="rounded mb-4 w-full h-40 object-cover">
                    @endif
                    ...
                    <form action="{{ route('cart.add', $course['id']) }}" method="POST">
                        @csrf
                        <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded w-full mt-2 hover:bg-blue-600">
                            Add to Cart
                        </button>
                    </form>
                </div>
