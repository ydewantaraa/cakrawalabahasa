<div class="bg-white rounded-lg shadow-lg w-full max-w-md sm:max-w-2xl z-50 p-4 sm:p-6" x-data="{
    form: {
    ...
    <div class="flex justify-end gap-2 pt-4">
        ...
        <button type="submit" form="update-sub-{{ $sub->id }}"
            class="px-3 sm:px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 text-sm">
            Update
        </button>
        ...
        <form action="{{ route('sub-course-service.destroy', $sub->id) }}" method="POST"
            ...
            <button type="submit" class="px-3 sm:px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600 text-sm">
                Hapus
            </button>
        </form>
