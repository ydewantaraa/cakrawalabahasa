    ...
    <div x-data="{ open: true }" class="relative md:flex -mx-6 pt-16">
        ...
        <div class="flex-1 min-h-screen transition-all duration-300"
            ...
            <div class="max-w-full sm:max-w-5xl lg:max-w-7xl mx-auto p-4 sm:p-6 md:p-8">
                ...
                @switch($tab)
                    @case('my-classes')
                        @include('student.tabs.my-class')
                    @break

                    @case('payment-history')
                        @include('student.tabs.payment-history')
                    @break

                    @case('favorite-classes')
                        @include('student.tabs.favorite-class')
                    @break

                    @default
                        @include('student.tabs.overview')
                @endswitch
            </div>
        </div>
    </div>
